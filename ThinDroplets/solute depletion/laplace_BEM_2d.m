function [c, dcdn] = laplace_BEM_2d(N,L,H,Abot,NGL)
...
for j = 1:4*N
    ...
    for i=2*N+1:3*N
        if i == j
            [~, dlp] = rect_sdlp(midx(j), midy(j), elmx(i,1), elmy(i,1), elmx(i,2), elmy(i,2), NGL, 1);
            M(j,i) = dlp -0.5;
        else
            [~, dlp] = rect_sdlp(midx(j), midy(j), elmx(i,1), elmy(i,1), elmx(i,2), elmy(i,2), NGL, 0);
            M(j,i) = dlp;
        end
    end
    for i=3*N+1:4*N
