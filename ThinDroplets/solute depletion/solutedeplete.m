...
Abot = zeros(N,1);
Abot(floor(0.5*(N-1))+1-floor((N-1)/L):ceil(0.5*(N-1))+1+floor((N-1)/L))=-1;
Abot1 = zeros(Nx,1);
Abot1(floor(0.5*(Nx))-floor((Nx)/L)+1:ceil(0.5*(Nx))+floor((Nx)/L))=-1;
%%
[crec4,frec4] = solute(dx, L, H, Abot,Abot1, 2, 4,10);
%%
%[crec10,frec10] = solute(dx, L, H, Abot,Abot1, 1, 10,10);
%%
